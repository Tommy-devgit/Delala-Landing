...
import { MARKETS, NAV_ITEMS } from "@/lib/constants";
import { MapPin, Mail, Phone, ShieldCheck } from "lucide-react";

export function Footer() {
  return (
    <footer className="bg-primary-hover text-canvas pt-16 pb-12 border-t border-line/20 select-none">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-12 mb-12">
          {/* Brand Column */}
          <div className="lg:col-span-2 space-y-4">
            <Link href="/" className="group">
              <span className="font-heading font-black text-2xl tracking-tight text-white group-hover:text-accent transition-colors">
                Delala
              </span>
            </Link>

            <p className="text-sm text-canvas/80 leading-relaxed max-w-sm">
            ...
            <div className="pt-2 text-xs text-accent font-semibold flex items-center gap-2">
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm text-canvas/75 font-medium">
              {MARKETS.map((market) => (
                <li key={market.slug}>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-canvas/75">
              ...
              <li className="pt-2 text-xs text-accent font-semibold flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-accent" />
                <span>Listings reviewed before publishing</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-line/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-canvas/60">
          <div>
