...
/**
 * A neighborhood. Coordinates are optional approximate area centres used only to
 * frame the map; when absent the picker falls back to the sub-city centre.
 */
type NeighborhoodSeed = string | { name: string; latitude?: number; longitude?: number };
  ...
  for (const city of citiesData) {
    ...
    for (const subCity of city.subCities) {
      ...
      for (const entry of subCity.neighborhoods || []) {
        ...
        const { name, latitude, longitude } = neighborhood as {
          name: string;
          latitude?: number;
          longitude?: number;
        };
