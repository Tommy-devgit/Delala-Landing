...
async function main() {
    ...
    for (const city of citiesData) {
        ...
        if (!cityRecord) {
            ...
            if (cityRecord.latitude === null || cityRecord.longitude === null) {
                await prisma.location.update({
                    ...
                    data: { latitude: city.latitude, longitude: city.longitude },
                });
        ...
        for (const subCity of city.subCities) {
            ...
            else if ((subCityRecord.latitude === null || subCityRecord.longitude === null) &&
                subCity.latitude !== undefined &&
                subCity.longitude !== undefined) {
                await prisma.location.update({
                    where: { id: subCityRecord.id },
                    data: { latitude: subCity.latitude, longitude: subCity.longitude },
                });
